...
found:
  ...
  /**************Solutions of lab pgtbl***************/

  // Create and initialize a kernel page table.
...
freeproc(struct proc *p)
{
  ...
  if(p->kstack)
      proc_freekstack(p->k_pagetable, p->kstack);
  if(p->k_pagetable)
    proc_freekernelpgtb(p->k_pagetable);
...
proc_kvminit(struct proc *p)
{
  ...
  // map the trapframe of the user pagetable to just below the trampoline.
  proc_kvmmap(k_pagetable, TRAPFRAME, (uint64)(p->trapframe), PGSIZE, PTE_R | PTE_W);
...
void
proc_freekernelpgtb(pagetable_t k_pagetable)
{
  kvmfreepgtb(k_pagetable);
}

// Free a process's kernel stack and the 
// physical memory it refers to.
void
proc_freekstack(pagetable_t k_pagetable, uint64 kstack)
{
    freekstack(k_pagetable, kstack);
}
...
userinit(void)
{
  ...
  copyusrmap(p->k_pagetable, p->pagetable, 0, p->sz);
...
growproc(int n)
{
  ...
  copyusrmap(p->k_pagetable, p->pagetable, p->sz, sz);
...
fork(void)
{
  ...
  copyusrmap(np->k_pagetable, np->pagetable, 0, np->sz);
  ...
  for(;;){
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE) {
        ...
        // Load the global kernel page table into satp register.
        kvminithart();
